...
class DialogueRNNCellPersonSpecific(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, listener_state=False,
        ...
        self.p_cell_a = nn.GRUCell(D_m+D_g,D_p) # 100+500, 500
        self.p_cell_b = nn.GRUCell(D_m+D_g,D_p) # 100+500, 500
    ...
    def _select_parties(self, X, indices):
        ...
        for idx, j in zip(indices, X):
            ...
            if idx == 0:
                q0_sel_a.append(j[idx].unsqueeze(0))
            else:
                q0_sel_b.append(j[idx].unsqueeze(0))
        q0_sel = torch.cat(q0_sel,0)
        q0_sel_a = torch.cat(q0_sel_a,0)
        q0_sel_b = torch.cat(q0_sel_b,0)
        return q0_sel, q0_sel_a, q0_sel_b
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        ...
        q0_sel, q0_sel_a, q0_sel_b = self._select_parties(q0, qm_idx) # n, self.D_p; batch - n, self.D_p
...
class BiModelCRF(nn.Module):
    ...
    def forward(self, U, qmask, umask,label,att2=True):
        """
        U -> seq_len, batch, D_m
        qmask -> seq_len, batch, party
        """
...
class WithUserSpecificCRF(BiModel):

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.crf_1 = CRF(kwargs['n_classes'])
        self.crf_2 = CRF(kwargs['n_classes'])
    
    def forward(self, U, qmask, umask, label, **kwargs):
        # 输出的内容:
        # if att2:
        #     return log_prob, alpha, alpha_f, alpha_b
        # else:
        #     return log_prob, [], alpha_f, alpha_b
        output = super().forward(U, qmask, umask, **kwargs)
        log_prob = output[0]
        ...
        loss_1 = self.crf_1(log_prob_1, label_1, umask_1.T.bool())
        loss_2 = self.crf_2(log_prob_2, label_2, umask_2.T.bool())
        loss = 0.5 * loss_2 + 0.5 * loss_1
